<template>
  <form v-if="v">
    <ValidatedInputGroup v-slot="{ isValid }" :errors="v.email.$errors">
      <TextInput v-model="editableUser.email" autocomplete="username" :placeholder="$t('form_fields.email')" type="email"
        :has-errors="!isValid" required :readonly="isSubmitting" data-test-id="login-email" @change="v.email.$touch()" />
    </ValidatedInputGroup>

    <ValidatedInputGroup v-slot="{ isValid }" :errors="v.password.$errors">
      <TextInput v-model="editableUser.password" autocomplete="current-password" :placeholder="$t('form_fields.password')"
        type="password" :has-errors="!isValid" required :readonly="isSubmitting" data-test-id="login-password"
    ...
    <div class="py-2 text-right">
      ...
      }" raw class="group relative text-sm text-gray-800 hover:text-black">
    ...
    <AppButton class="w-full" :disabled="isSubmitting" :loading="isSubmitting" data-test-id="login-submit"
      @click="onSubmit()">
      {{ $t('global.sign_in') }}
    </AppButton>
  </form>
